...
export default function Portfolio(){
 ...
 <button className="project-visual" onClick={()=>setSelected(p)} aria-label={'Explore '+p.name}><img src={'/images/'+p.slug+'.webp'} alt={p.name+' project photograph'} width="1280" height="850" loading="lazy"/><span className="project-corner mono">0{i+1} / {p.sector.toUpperCase()}</span><span className="project-open"><Arrow diagonal/></span></button>
 ...
 {selected&&<><button className="dialog-close round-button" onClick={()=>setSelected(null)} aria-label="Close project details">×</button><img className="dialog-project-image" src={'/images/'+selected.slug+'.webp'} alt={selected.name+' project photograph'} width="1200" height="700"/><div className="dialog-project-content"><span className="mono">{selected.sector.toUpperCase()} / COMPLETED PROJECT</span><h2>{selected.name}</h2><p>{selected.location}</p><dl><div><dt>System</dt><dd>{selected.system}</dd></div><div><dt>Contracting client</dt><dd>{selected.client}</dd></div><div><dt>Capacity</dt><dd>{selected.capacity} <small>tonnes*</small></dd></div></dl>
 {selected.slug==='stanley'&&<p className="project-note">Stanley Government Hospital: 950 TR delivered in 45 days.</p>}
